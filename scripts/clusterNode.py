...
class clusterNode:
    def __init__(self,linkage,agglo_cutoff):
        ...
        self.pointArray = []
        self.maxAmountOfPoints = 50 # maximum amount of points in the array
        self.targetLocked = 0
        self.targetPoint = []
        # Initialize rosnode
        self.tf_listener = tf.TransformListener()
        targetSub = rospy.Subscriber('/pointcloudregistration/target', PointStamped, self.processPoints, queue_size=10)
        publisher = rospy.Publisher('staircase_detection_visualization', MarkerArray, queue_size=10)
        # Create markers
        self.targetMarkers = markerArrayRVIZ(50,0.1,[1,0,0],'/map',publisher)
        self.goalMarker = markerArrayRVIZ(1,0.2,[0,1,0],'/map',publisher)
    ...
    def processPoints(self,pointStamped):
        ...
        self.pointArray.append([nP.point.x,nP.point.y,nP.point.z])
        ...
        if len(self.pointArray)>self.maxAmountOfPoints:
            ...
            if self.targetLocked==0:
                self.targetPoint = agglomerativeClustering(self.pointArray,self.linkage,self.agglo_cutoff,self.verbosity)
...
if __name__ == "__main__":
    rospy.init_node('target_processing_ardrone')
    cluster = clusterNode('t',150.0)